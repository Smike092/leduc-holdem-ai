"""
The content of this file needs to replace rlcard.games.leducholdem.game to make expectiminimax works
"""
...
class LeducholdemGame(Game):
    ...
    def init_game(self):
        ...
        self.dealer_expecti = Dealer(self.np_random)
        ...
        # maj dealer_expecti
        self.dealer_expecti.deck.remove(self.players[0].hand)
    ...
    def get_payoffs(self):
        ...
        if self.planning:
            payoffs = 0
            num_possibilities = 0
            # we calculate the average of the payoff for every card that can be the second player's card
            for card in self.dealer_expecti.deck:
                if card != self.public_card:
                    num_possibilities += 1
                    self.players[1].hand = card
                    chips_payoffs = self.judger.judge_game(self.players, self.public_card)
                    payoffs += np.array(chips_payoffs) / (self.big_blind)
            return payoffs / num_possibilities
        chips_payoffs = self.judger.judge_game(self.players, self.public_card)
        payoffs = np.array(chips_payoffs) / (self.big_blind)
        return payoffs
